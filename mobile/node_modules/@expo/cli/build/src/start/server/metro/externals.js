    ...
    for (const moduleId of NODE_STDLIB_MODULES){
        ...
        if (!_fs.default.existsSync(shimPath)) {
            try {
                ...
                // Skip directory creation on Windows if it contains problematic characters
                if (error.code === 'ENOENT' && moduleId.includes(':')) {
                    console.log(`Skipping ${moduleId} directory creation on Windows`);
                    continue;
                }
                throw error;
            }
